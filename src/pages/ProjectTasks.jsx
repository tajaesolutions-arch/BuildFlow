...
import { addAttachmentMetadata, canReviewQuality, createInspection, createPunchItem, openPunchBlocksCompletion } from '../lib/qualityControl'
...
export function ProjectTasks() {
  ...
  const [photo, setPhoto] = useState({ fileName: '', filePath: '', attachmentType: 'progress_photo' })
  ...
  async function addProofPhoto(event) {
    event.preventDefault(); if (!selected) return
    try { await addAttachmentMetadata({ workspaceId: selected.task.workspace_id, projectId: selected.task.project_id, userId: user.id, attachment: { ...photo, taskId: selected.task.id, fileType: 'image' } }); setPhoto({ fileName: '', filePath: '', attachmentType: 'progress_photo' }); setSelected(await fetchTaskDetail(selected.task.id)) } catch (err) { setError(err.message) }
  }

  async function createTaskInspection() {
    if (!selected) return
    try { await createInspection({ workspaceId: selected.task.workspace_id, projectId: selected.task.project_id, userId: user.id, inspection: { title: `Inspection: ${selected.task.title}`, taskId: selected.task.id, stageId: selected.task.stage_id || '', projectAreaId: selected.task.project_area_id || '', assignedTo: user.id }, checklistItems: [] }); setSelected(await fetchTaskDetail(selected.task.id)); await load() } catch (err) { setError(err.message) }
  }

  async function createTaskPunchItem() {
    if (!selected) return
    try { await createPunchItem({ workspaceId: selected.task.workspace_id, projectId: selected.task.project_id, userId: user.id, item: { title: `Correction: ${selected.task.title}`, taskId: selected.task.id, stageId: selected.task.stage_id || '', projectAreaId: selected.task.project_area_id || '', priority: 'high', assignedTo: selected.task.assigned_to || '' } }); setSelected(await fetchTaskDetail(selected.task.id)); await load() } catch (err) { setError(err.message) }
  }
